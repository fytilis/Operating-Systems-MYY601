...
#include <assert.h>
#include "db.h"
#include <pthread.h> // gia ta rwlocks
#include "indexer.h"
#include "utils.h"
#include "log.h"
...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    pthread_rwlock_init(&self->rwlock, NULL); // lock gia na mhn ginei patatra
...
void db_close(DB *self)
{
    ...
    pthread_rwlock_destroy(&self->rwlock); // katharizoume to lock sto telos
...
{
    pthread_rwlock_wrlock(&self->rwlock); // kleidwma gia eggrafh
    ...
    if (memtable_needs_compaction(self->memtable))
	{
	    ...
	    skiplist_acquire(self->memtable->list);
	    sst_merge(self->sst, self->memtable);
	    self->memtable->compacted = 1;
    ...
    pthread_rwlock_unlock(&self->rwlock); // telos eggrafhs
    return result;
}

int db_get(DB* self, Variant* key, Variant* value)
{
    pthread_rwlock_rdlock(&self->rwlock); // kleidwma gia anagnwsh
