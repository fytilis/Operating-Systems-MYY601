				  ...
				  unsigned int flags)
{
	...
	// Το όνομα που ψάχνουμε είναι στο dentry->d_name.name
    	const char *lookup_name = "?";
    	if (dentry && dentry->d_name.name) {
        	lookup_name = dentry->d_name.name;
    	}
    	// Το όνομα του καταλόγου που ψάχνουμε είναι στο dir->i_dentry->d_name.name (πιο περίπλοκο)
    	// Ας αρκεστούμε στο όνομα που ψάχνουμε προς το παρόν.

    	snprintf(log_msg, sizeof(log_msg), "FAT_LOOKUP | name=%s\n", lookup_name);
		      ...
		      struct dentry *dentry, umode_t mode)
{
	...
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	{
    		...
    		const char *dirname = "?";
    		// Οι παράμετροι dir, dentry, mode πρέπει να είναι διαθέσιμες
    		if (dentry && dentry->d_name.name) dirname = dentry->d_name.name;
    		snprintf(log_msg, sizeof(log_msg), "FAT_MKDIR | parent_inode=%lu | name=%s | mode=%o\n",
             	dir->i_ino, dirname, mode);
    		lkl_fat_log_write(log_msg);
	}
	ts = current_time(dir);
