			...
			struct page **pagep, void **fsdata)
{
	int err;
	char log_msg[300];
    	// Παίρνουμε το inode από το mapping
    	struct inode *inode = mapping->host;
    	// Παίρνουμε τη θέση από την παράμετρο pos
    	// Παίρνουμε το μήκος από την παράμετρο len

    	snprintf(log_msg, sizeof(log_msg), "FAT_WRITE_BEGIN | inode=%lu | pos=%lld | len=%u\n",
             inode ? inode->i_ino : 0, // Παίρνουμε τον αριθμό inode (ή 0 αν NULL)
             pos,
             len);
			...
			struct page *pagep, void *fsdata)
{
	...
	if (err < len)
    	...
    	// Παίρνουμε τη θέση (pos), το αρχικό μήκος (len) και τα bytes που γράφτηκαν (copied)

    	snprintf(log_msg, sizeof(log_msg), "FAT_WRITE_END | inode=%lu | pos=%lld | len=%u | copied=%u\n",
             inode ? inode->i_ino : 0,
             pos,
             len,
             copied);
...
{	
	...
	// Καταγράφουμε το inode και την παλιά του θέση (i_pos πριν μηδενιστεί)
    	loff_t old_pos = MSDOS_I(inode)->i_pos; // Κράτα την παλιά θέση

    	snprintf(log_msg, sizeof(log_msg), "FAT_DETACH | inode=%lu | old_i_pos=%lld\n",
        	inode->i_ino, old_pos);
    	lkl_fat_log_write(log_msg);
...
int fat_fill_inode(struct inode *inode, struct msdos_dir_entry *de)
{
	...
	snprintf(log_msg, sizeof(log_msg), "FAT_FILL_INODE | inode=%lu | de_attr=0x%x | de_size=%u | de_start=%u\n",
					 inode ->i_ino,
					 (unsigned int) de->attr,
					 (unsigned int)le32_to_cpu(de->size),
					 (unsigned int)fat_get_start(sbi,de));
	lkl_fat_log_write(log_msg);
...
static void fat_evict_inode(struct inode *inode)
{
	...
	truncate_inode_pages_final(&inode->i_data);
    	...
    	snprintf(log_msg, sizeof(log_msg), "FAT_EVICT_INODE | inode=%lu | i_nlink=%u | i_size=%lld\n",
             inode->i_ino,
             inode->i_nlink, // Αριθμός συνδέσμων (links)
             inode->i_size); // Μέγεθος αρχείου
    	lkl_fat_log_write(log_msg);
	if (!inode->i_nlink) {
		inode->i_size = 0;
